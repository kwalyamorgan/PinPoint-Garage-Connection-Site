    ...
    <div className={embedded ? 'border-y border-border bg-background text-foreground' : 'fixed inset-0 z-50 overflow-y-auto bg-background text-foreground'}>
      ...
      <main className="mx-auto max-w-6xl px-4 py-6 md:px-8 md:py-10">
        ...
        <section className="min-w-0">
          {message && <div className="mb-4 flex items-center justify-between rounded border border-primary/30 bg-primary/10 px-4 py-3 text-sm text-foreground"><span>{message}</span><button onClick={() => setMessage('')}><X size={16} /></button></div>}
          ...
          {!loading && tab === 'services' && (
            <div className="space-y-5">
              ...
              {activeServices.length === 0 ? <div className="rounded border border-border bg-card p-10 text-center text-muted-foreground">No providers are available for this service yet.</div> : <div className="grid gap-4 md:grid-cols-2">{activeServices.map(provider => <article key={provider.id} className="overflow-hidden rounded border border-border bg-card"><div className="flex gap-4 p-4"><img src={provider.img} alt={provider.name || provider.model || 'Provider'} className="h-24 w-28 rounded object-cover" /><div className="min-w-0"><h3 className="font-bold">{provider.name || provider.model || 'Provider'}</h3><p className="text-sm text-primary">{provider.specialty || provider.type || serviceType}</p><p className="mt-1 flex items-center gap-1 text-sm text-muted-foreground"><MapPin size={14} />{provider.location || provider.area || provider.pickup || 'Nairobi'}</p><p className="mt-1 text-xs text-muted-foreground">{provider.availability || (provider.isAvailable === false ? 'Unavailable' : 'Available now')}</p></div></div><div className="flex gap-2 border-t border-border p-4"><button onClick={() => setSelectedProvider({ ...provider, serviceType })} className="flex-1 rounded border border-primary px-3 py-2 text-sm font-semibold text-primary">View details</button><button onClick={() => setSelectedProvider({ ...provider, serviceType })} className="flex-1 rounded bg-primary px-3 py-2 text-sm font-semibold text-white">Book now</button></div></article>)}</div>}
