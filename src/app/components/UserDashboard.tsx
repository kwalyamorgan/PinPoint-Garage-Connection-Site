...
import MechanicRequestDialog from './MechanicRequestDialog';
...
const requestStatusLabels: Record<string, string> = {
  pending: 'Request received', reviewed: 'Being reviewed', approved: 'Arrival time confirmed', 'en-route': 'Mechanic en route', arrived: 'Mechanic arrived', completed: 'Completed', rejected: 'Request declined',
};
...
interface Booking {
  ...
  bookingChannel?: 'site' | 'whatsapp' | 'mechanic-request';
    ...
    <div className={embedded ? 'border-y border-border bg-background text-foreground' : 'fixed inset-0 z-50 overflow-y-auto bg-background text-foreground'}>
      <header className="sticky top-0 z-10 border-b border-border bg-background/95 px-4 py-4 backdrop-blur md:px-8">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4">
          <div>
            <p className="portal-kicker">PinPoint / driver desk</p>
      ...
      <main className="mx-auto max-w-6xl px-4 py-6 md:px-8 md:py-10">
        <div className="portal-surface mb-6 flex flex-col gap-3 rounded-xl p-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-col gap-2 sm:flex-row"><button onClick={() => setMechanicRequestOpen(true)} className="flex items-center justify-center gap-2 rounded bg-primary px-4 py-2.5 text-sm font-bold text-white"><Wrench size={16} /> Request a mechanic</button><select value={tab} onChange={event => chooseAction(event.target.value)} className="w-full rounded border border-border bg-secondary px-3 py-2.5 text-sm font-semibold text-foreground outline-none sm:w-52">
        ...
        {bookings.some(booking => booking.bookingChannel === 'mechanic-request' && !['completed', 'rejected'].includes(booking.status)) && <button onClick={() => setTab('bookings')} className="portal-surface mb-6 flex w-full items-center justify-between gap-4 rounded-xl border-primary/35 p-4 text-left hover:border-primary/70"><span><span className="portal-kicker">Active roadside request</span><span className="mt-1 block text-sm font-bold">Your mechanic request is being coordinated</span></span><span className="rounded-full bg-primary/15 px-3 py-1 text-xs font-bold text-primary">Track progress</span></button>}
        ...
        <section className="min-w-0">
          ...
          {!loading && tab === 'bookings' && (
            <div className="space-y-5">
              ...
              {bookings.length === 0 ? <div className="rounded border border-border bg-card p-10 text-center text-muted-foreground">No bookings yet. Browse providers and choose View Details to request a service.</div> : bookings.map(booking => (
                <article key={booking.id} className={`portal-surface rounded-xl p-5 ${booking.bookingChannel === 'mechanic-request' ? 'border-primary/40' : ''}`}>
                  <div className="flex flex-col justify-between gap-3 sm:flex-row"><div><h3 className="font-bold">{booking.providerName}</h3><p className="text-xs uppercase tracking-wider text-primary">{booking.requestService || booking.serviceType}</p><p className="mt-1 text-sm text-muted-foreground">Requested {booking.dateRequested ? new Date(booking.dateRequested).toLocaleString() : 'recently'}</p></div><span className={`flex h-fit items-center gap-1 rounded px-2 py-1 text-xs font-bold uppercase ${booking.status === 'approved' || booking.status === 'completed' ? 'bg-green-500/15 text-green-400' : booking.status === 'rejected' ? 'bg-red-500/15 text-red-400' : 'bg-yellow-500/15 text-yellow-400'}`}>{booking.status === 'approved' || booking.status === 'completed' ? <CheckCircle size={13} /> : booking.status === 'rejected' ? <XCircle size={13} /> : <Clock size={13} />}{booking.status}</span></div>
                  {booking.bookingChannel === 'mechanic-request' && <div className="mt-4 grid gap-3 rounded bg-secondary p-3 text-sm sm:grid-cols-2"><p className="flex items-center gap-2"><MapPin size={15} />Live location: {booking.requestedLocation || 'Awaiting location'}</p><p><strong>Time of arrival:</strong> {booking.dateApproved ? new Date(booking.dateApproved).toLocaleString() : 'Admin is scheduling this'}</p><p><strong>Vehicle:</strong> {[booking.vehicleMake, booking.vehicleModel, booking.vehicleYear].filter(Boolean).join(' ') || 'Not provided'}{booking.licensePlate ? ` · ${booking.licensePlate}` : ''}</p><p><strong>Progress:</strong> {requestStatusLabels[booking.status] || booking.status}</p></div>}
          ...
          {!loading && tab === 'services' && (
            <div className="space-y-5">
              ...
              {activeServices.length === 0 ? <div className="rounded border border-border bg-card p-10 text-center text-muted-foreground">No services are available yet.</div> : <div className="grid gap-4 md:grid-cols-2">{activeServices.map(provider => <article key={provider.id} className="overflow-hidden rounded border border-border bg-card"><div className="flex flex-col gap-4 p-4 sm:flex-row"><img src={provider.img} alt={provider.name || provider.model || 'Service'} className="h-40 w-full rounded object-cover sm:h-24 sm:w-28 sm:shrink-0" /><div className="min-w-0"><h3 className="font-bold">{provider.name || provider.model || 'Service'}</h3><p className="text-sm text-primary">{provider.specialty || provider.type || serviceType}</p><p className="mt-1 flex items-center gap-1 text-sm text-muted-foreground"><MapPin size={14} />{provider.location || provider.area || provider.pickup || 'Nairobi'}</p><p className="mt-1 text-xs text-muted-foreground">{provider.availability || 'Available now'}</p></div></div><div className="border-t border-border p-4"><button onClick={() => setSelectedProvider({ ...provider, serviceType })} className={`w-full rounded px-3 py-2 text-sm font-semibold ${serviceType === 'garage' ? 'border border-border text-muted-foreground' : 'bg-primary text-white'}`}>{serviceType === 'garage' ? 'View garage details' : 'Book now'}</button></div></article>)}</div>}
              {selectedProvider && <div className="rounded border border-primary/40 bg-card p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-widest text-primary">{providerType === 'garage' ? 'Garage details' : 'Service details'}</p><h2 className="text-xl font-black">{providerName}</h2><p className="text-sm text-muted-foreground">{selectedProvider.location || selectedProvider.area || selectedProvider.pickup || 'Nairobi'} · {providerType}</p></div><button onClick={() => setSelectedProvider(null)} aria-label="Close service details"><X size={18} /></button></div><div className="mt-4 grid gap-2 text-sm text-muted-foreground sm:grid-cols-2">{selectedProvider.specialty && <p><strong className="text-foreground">Specialty:</strong> {selectedProvider.specialty}</p>}{selectedProvider.description && <p><strong className="text-foreground">Description:</strong> {selectedProvider.description}</p>}{selectedProvider.price && <p><strong className="text-foreground">Price:</strong> {selectedProvider.price}</p>}{selectedProvider.phone && <p><strong className="text-foreground">Phone:</strong> {selectedProvider.phone}</p>}{selectedProvider.availability && <p><strong className="text-foreground">Availability:</strong> {selectedProvider.availability}</p>}</div>{providerType !== 'garage' && <><textarea value={bookingDescription} onChange={e => setBookingDescription(e.target.value)} placeholder={`What do you need from ${providerName}?`} className="mt-4 min-h-24 w-full rounded border border-border bg-secondary px-3 py-2 text-sm" /><div className="mt-3 flex flex-wrap gap-2"><button disabled={saving} onClick={createBooking} className="rounded bg-primary px-4 py-2 text-sm font-semibold text-white">{saving ? 'Sending...' : 'Book from site'}</button><button disabled={saving} onClick={bookViaWhatsapp} className="flex items-center gap-2 rounded bg-[#25D366] px-4 py-2 text-sm font-semibold text-white"><MessageCircle size={16} />Book via WhatsApp</button></div></>}</div>}
              {selectedProvider && <div className="rounded border border-primary/40 bg-card p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-widest text-primary">{providerType === 'garage' ? 'Garage details' : 'Service details'}</p><h2 className="text-xl font-black">{providerName}</h2><p className="text-sm text-muted-foreground">{selectedProvider.location || selectedProvider.area || selectedProvider.pickup || 'Nairobi'} · {providerType}</p></div><button onClick={() => setSelectedProvider(null)} aria-label="Close service details"><X size={18} /></button></div><div className="mt-4 grid gap-2 text-sm text-muted-foreground sm:grid-cols-2">{selectedProvider.specialty && <p><strong className="text-foreground">Specialty:</strong> {selectedProvider.specialty}</p>}{selectedProvider.description && <p><strong className="text-foreground">Description:</strong> {selectedProvider.description}</p>}{selectedProvider.serviceDetails && <p><strong className="text-foreground">Services and charges:</strong> {selectedProvider.serviceDetails}</p>}{selectedProvider.requirements && <p><strong className="text-foreground">Requirements:</strong> {selectedProvider.requirements}</p>}{selectedProvider.price && <p><strong className="text-foreground">Price:</strong> {selectedProvider.price}</p>}{selectedProvider.phone && <p><strong className="text-foreground">Phone:</strong> {selectedProvider.phone}</p>}{selectedProvider.availability && <p><strong className="text-foreground">Availability:</strong> {selectedProvider.availability}</p>}</div>{providerType !== 'garage' && <><textarea value={bookingDescription} onChange={e => setBookingDescription(e.target.value)} placeholder={`What do you need from ${providerName}?`} className="mt-4 min-h-24 w-full rounded border border-border bg-secondary px-3 py-2 text-sm" /><div className="mt-3 flex flex-wrap gap-2"><button disabled={saving} onClick={createBooking} className="rounded bg-primary px-4 py-2 text-sm font-semibold text-white">{saving ? 'Sending...' : 'Book from site'}</button><button disabled={saving} onClick={bookViaWhatsapp} className="flex items-center gap-2 rounded bg-[#25D366] px-4 py-2 text-sm font-semibold text-white"><MessageCircle size={16} />Book via WhatsApp</button></div></>}</div>}
            </div>
          )}
      ...
      <MechanicRequestDialog open={mechanicRequestOpen} onOpenChange={setMechanicRequestOpen} user={user} onRequireLogin={() => setMessage('Please sign in again to submit a mechanic request.')} onSubmitted={() => { setTab('bookings'); load(); }} />
    </div>
  );
}
